...
O toque de ar é um gesto de toque com a mão mantida na vertical, semelhante a um clique do mouse ou selecionar. Isso é usado na maioria das experiências de HoloLens para o equivalente a um "clique" em um elemento de interface do usuário depois de direcioná-lo com [olhar](gaze-and-commit.md). É uma ação universal que você aprende uma vez e, em seguida, se aplica em todos os seus aplicativos. Outras maneiras de executar SELECT são pressionar o botão único em um selecionador de [HoloLens](hardware-accessories.md#hololens-clicker) ou falando sobre o comando de voz "Select".

![estado pronto para gestos de mão em](images/image9.png) de HoloLens<br>
...
*Como executar um toque de ar: eleve o dedo do seu índice à posição pronta, pressione o dedo para baixo para tocar ou selecionar e, em seguida, faça o backup para o lançamento.*
...
<td>Fechar e abrir dedos indicador e polegar e manter</td><td><p>Manter é simplesmente manter a posição do dedo para baixo no gesto de fechar e abrir dedos indicador e polegar. A combinação de toque e espera de ar permite uma variedade de &quot;mais complexas clique e arraste&quot; interações quando combinadas com movimento de ARM, como a seleção de um objeto em vez de ativá-lo ou &quot;MouseDown&quot; interações secundárias como mostrar um menu de contexto.</p><p>No entanto, tenha cuidado ao projetar o uso desse gesto, pois os usuários podem estar propensos a relaxar suas posturas de mão no decorrer de qualquer gesto estendido.</p></td>
</tr><tr>
