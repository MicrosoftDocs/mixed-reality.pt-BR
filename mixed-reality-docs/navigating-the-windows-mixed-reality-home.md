...
Para iniciar um aplicativo, selecione-o em Iniciar. O menu iniciar desaparecerá e o aplicativo será aberto no modo de posicionamento, como uma janela 2D ou um [modelo 3D](implementing-3d-app-launchers.md).

Para executar o aplicativo, você precisará colocá-lo em sua página inicial:
1. Use seu [olhar](gaze.md) ou controlador para posicionar o aplicativo onde desejar. Ele será ajustado automaticamente (em tamanho e posição) para estar em conformidade com o espaço onde você o coloca.
...
Aplicativos diferentes podem ter opções adicionais na barra de aplicativos. Por exemplo, o Microsoft Edgetem opções de rolagem, *arrastar*e *zoom* . 
...
1. Abrir o reticle de teleportação.
   * Usando [controladores de movimento](motion-controllers.md): Pressione o Thumbstick para frente e segure-o nessa posição.
...
2. Coloque o reticle onde você deseja teleport.
   * Usando [controladores de movimento](motion-controllers.md): Incline o controlador (no qual você está mantendo o Thumbstick Forward) para mover o reticle.
   * Usando um controlador Xbox: Use seu [olhar](gaze.md) para mover o reticle.
...
* Usando os [controladores de movimento](motion-controllers.md): clique no botão de Thumbstick e segure e, em seguida, mova o Thumbstick na direção que você deseja "andar".
...
* Pressionar o **botão** a executa uma seleção e age como o gesto de [toque do ar](gestures.md#air-tap) .
* Pressionar o botão **guia** abre o [menu iniciar](navigating-the-windows-mixed-reality-home.md#start-menu) e atua como o gesto de [cair](gestures.md#bloom) .
...
* Pressionar o botão **esquerdo** do mouse executa uma seleção e age como o gesto de [toque do ar](gestures.md#air-tap) .
...
* Pressionar a tecla **Windows** no teclado abre o [menu iniciar](navigating-the-windows-mixed-reality-home.md#start-menu) e age como o gesto de [cair](gestures.md#bloom) .
* Quando [nuvens](gaze.md) em um aplicativo de área de trabalho 2D, você pode **clicar** para selecionar, clicar com o **botão direito do mouse** para exibir menus de contexto e usar a **roda** de rolagem para rolar (assim como na área de trabalho do seu PC).

## <a name="cortana"></a>Cortana

O [Cortana](voice-input.md#hey-cortana) é o seu assistente pessoal no Windows Mixed Reality, assim como no PC e no telefone. O HoloLens tem um microfone interno, mas os headsets de imersão podem exigir hardware adicional. Use a Cortana para abrir aplicativos, reinicie o dispositivo, verifique as coisas online e muito mais. Os desenvolvedores também podem optar por [integrar](https://dev.windows.com/cortana) o Cortana em suas experiências.

Você também pode usar comandos de voz para colocar em sua casa. Por exemplo, aponte para um botão (usando [olhar](gaze.md) ou um controlador, dependendo do dispositivo) e diga "Select". Outros comandos de voz incluem "ir para a casa", "maior", "menor", "fechar" e "entrar em frente".
...
* **Gestos de aprendizagem** (HoloLens) ou **Aprenda a realidade misturada** (headsets de imersão) para saber mais sobre como usar seu dispositivo
...
* [Implementando iniciadores de aplicativos 3D](implementing-3d-app-launchers.md)
